...
describe('useQuote on Arbitrum', () => {
  it('swap Quote on Arbitrum  from eth to usdc', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(swrData.data?.sellAmount).toEqual('1000000000000000')
      expect(swrData.data?.sellTokenAddress.toLowerCase()).toEqual(NativeCoinAddress.toLowerCase())

      expect(swrData.data?.buyTokenAddress.toLowerCase()).toEqual(USDC_IDS_TO_ADDR[42161].toLowerCase())
    })
  })

  it('dont swap Quote onArbitrum  do not nee swap', async () => {
    const { result } = renderHook(() => useQuote(false, true, '1000000000000000'), { wrapper })
...
describe('useQuote on Avalanche', () => {
  it('swap Quote on Avalanche  from usdc to aave ', async () => {
    const { result } = renderHook(() => useQuote(true, false, '1654819'), { wrapper })
    ...
    await waitFor(async () => {
      const swrData = await result.current
      expect(swrData.data?.sellTokenAddress).toEqual(USDC_IDS_TO_ADDR[43114].toLowerCase())
      expect(swrData.data?.buyTokenAddress).toEqual(AAVEToken?.address.toLowerCase())
    })
  })

  it('swap Quote on Avalanche  do not need swap ', async () => {
    const { result } = renderHook(() => useQuote(false, false, '1654819'), { wrapper })
